...
checkinputs <- function(input,session,rv){
  # --- Number of Turbines
  ...
  if(length(c_biomPars) > 0){
    ...
    c_biomParVals <- tibble(inputTag = names(c_biomPars), inputVal = unlist(c_biomPars)) %>%
      dplyr::filter(!str_detect(inputTag, pattern= "flType_tp")) %>%
      mutate(inputVal = as.numeric(inputVal)) %>%
      mutate(inputTags_split = stringr::str_split(inputTag, "_"),
             specLabel = map_chr(inputTags_split, function(x) paste(x[-c(1:4)], collapse = "_")),
             specName = stringr::str_replace_all(specLabel, pattern = "_", " "),
    ...
    c_biomParVals %>%
      dplyr::select(-inputTags_split, inputTag) %>%
      group_by(specLabel, par) %>%
      nest() %>%
      mutate(alerts = walk(data, function(x){
        input_E <- dplyr::filter(x, hyper == "E")$inputVal
        input_SD <- dplyr::filter(x, hyper == "SD")$inputVal
  ...
  names(isolate(input)) %>%
    stringr::str_subset(., pattern = "numInput_") %>%
    map(function(x){
      val <- isolate(input[[x]])
      if(!is.na(val)){
